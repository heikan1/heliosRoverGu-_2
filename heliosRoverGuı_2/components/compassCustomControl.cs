...
namespace compassdeneme.components
{
    public partial class compassCustomControl : Control
    {
        public compassCustomControl()
        {
            InitializeComponent();
        }
        ...
        public Bitmap DrawCompass(double degree)
        {
            ...
            SolidBrush blackBrush = new SolidBrush(BackColor);
            //Rectangle rect = new Rectangle(0, 0, compassImage.Width, compassImage.Height);
            g.FillEllipse(blackBrush, 0, 0, compassImage.Width, compassImage.Height);
            ...
            double markingsRadius = maxRadius - sizeRatio * 180; // yandaki 180 merkeze olan yakınlığını ayarlatıyo


            //Here we need to configuration of the marking texts and lines styles.
            //We will hold their sizes and colors here
            Font font1 = new Font("Arial", (float)(40 * sizeRatio));
            Font font2 = new Font("Arial", (float)(25 * sizeRatio));
            SolidBrush brushWhite = new SolidBrush(ForeColor);
            SolidBrush brushRed = new SolidBrush(Color.FromArgb(255, 0, 0));
            Pen penThick = new Pen(ForeColor, ((int)(sizeRatio) < 5 ? 5 : (int)(sizeRatio)));
            Pen penThickRed = new Pen(Color.FromArgb(255, 0, 0), ((int)(sizeRatio) < 5 ? 5 : (int)(sizeRatio)));
            Pen penThin = new Pen(ForeColor, ((int)(sizeRatio) < 2 ? 2 : (int)(sizeRatio)));
            ...
            for (int d = 0; d < 360; d += 5)
            {
                ...
                //g.DrawLine(penThickRed,, xCenter);
                if (d % 15 == 0)
                {
                    ...
                    {
                        if (d % 360 == 0) {
                            g.DrawLine(penThickRed, pOuter, pInner);
                        }
                        else
                        ...
                        if (d % 90 == 0)
                        {
                            switch (d)
                            {
                                case 0:
                                case 360:
                                    g.DrawString("N", font1, brushRed, pMarkings);
                                    break;
                            ...
                            g.DrawString(d.ToString(), font2, brushWhite, pMarkings);
                    }
                }
            ...
            //aracın yönünü gösteren oku çizmemi sağlayan kod
            double curAngle1 = (-90 +degree) / 180F * Math.PI;
            Point pCenter = new Point(xCenter, yCenter);
            Point pArrowHead = new Point((int)(outerradius * Math.Cos(curAngle1))+xCenter, (int)(outerradius * Math.Sin(curAngle1)) + yCenter);
            g.DrawLine(penThickRed, pCenter, pArrowHead);
            return compassImage;
        }
    }
}
